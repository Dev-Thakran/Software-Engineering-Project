...
function getAll(req, res) {
  ...
  const bookings = req.user.role === 'customer'
...
function create(req, res) {
  ...
  const booking = {
    id: 'b' + Date.now(), userId: req.user.id, userName: req.user.name,
    roomId, roomType: room.type, floor: room.floor,
    checkIn, checkOut, nights, total: nights * room.price,
