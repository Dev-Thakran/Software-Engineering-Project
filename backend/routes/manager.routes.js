...
router.get("/trend", authenticate, requireRole("manager"), (req, res) => {
	...
	for (let i = 13; i >= 0; i--) {
		...
		const count = bookings.filter(
			(b) => b.createdAt && b.createdAt.startsWith(date),
		).length;
...
router.get("/booking-mix", authenticate, requireRole("manager"), (req, res) => {
	...
	const mix = {};
	bookings.forEach((b) => {
		mix[b.roomType] = (mix[b.roomType] || 0) + 1;
	});
...
router.get("/staff", authenticate, requireRole("manager"), (req, res) => {
	...
	const result = staffUsers.map((u) => {
		const todayShift = shifts.find(
			(s) => s.staffId === u.id && s.date === today,
		);
		const attending = checkins.filter(
			(c) => c.handledBy === u.id && c.status === "checked-in",
		);
		...
		return {
			...
			email: u.email,
			shift: todayShift
				? `${todayShift.shiftType} (${todayShift.shiftStart} - ${todayShift.shiftEnd})`
				: "No shift today",
