document.addEventListener("DOMContentLoaded", function () {
        ...
        document.getElementById("date-display").textContent = `${weekday}, ${day} ${month}`;

        // Live clock
        function updateClock() {
            const timeOptions = { hour: "numeric", minute: "2-digit", hour12: true };
            document.getElementById("time-display").textContent =
                new Date().toLocaleTimeString("en-US", timeOptions).toLowerCase();
        }
        updateClock();
        ...
        async function loadShift() {
            if (!token) return;
            try {
                const res    = await fetch("/shifts/mine", { headers: { Authorization: "Bearer " + token } });
                const shifts = await res.json();
                const today  = new Date().toLocaleDateString("en-CA");
                const active = shifts.find(s => s.date === today && s.status === "active");
                ...
                document.getElementById("shift-hours").textContent = " (" + active.shiftStart + " — " + active.shiftEnd + ")";
                ...
                function updateCountdown() {
                    ...
                    if (remaining <= 0) {
                        ...
                        document.getElementById("shift-status").textContent    = "Shift complete";
                        return;
                    }
                    ...
                    document.getElementById("shift-countdown").textContent = `${pad(h)}:${pad(m)}:${pad(s)}`;
                    if (remaining < 1800000) {
                        document.getElementById("shift-countdown").style.color = "#9A826A";
                        document.getElementById("shift-status").textContent    = "Shift ending soon";
                    }
                }
                updateCountdown();
                setInterval(updateCountdown, 1000);
            } catch (err) {
                document.getElementById("shift-type").textContent = "Could not load shift";
            }
        }
        ...
        async function loadCheckIns() {
            if (!token) return;
            try {
                const res  = await fetch("/checkins/today", { headers: { Authorization: "Bearer " + token } });
        ...
        async function loadQueue() {
            if (!token) return;
            try {
                const res  = await fetch("/checkins/queue", { headers: { Authorization: "Bearer " + token } });
                const data = await res.json();
                ...
                document.getElementById("queue-list").innerHTML = `<p class="text-sm" style="color:#8B7355;">Could not load queue.</p>`;
            }
        }
        ...
        async function loadResidence() {
            if (!token) return;
            try {
                const res      = await fetch("/checkins/residence", { headers: { Authorization: "Bearer " + token } });
                ...
                document.getElementById("residence-list").innerHTML = `<p class="text-sm" style="color:#8B7355;">Could not load residence.</p>`;
            }
        }
